...
# ✅ Define file locations (relative to repo)
#   CSV derived from: data/Abi3_data_collate_main_figures.xlsx, sheet "Fig3H_Plaque_diffusivity_Int_Median"
...
# ✅ Data Subsets
group_1_data = df[df["Subject Name"] == "Abi3+/+_APPPS1"]["Intensity Median"]
group_2_data = df[df["Subject Name"] == "Abi3KI/+_APPPS1"]["Intensity Median"]
...
handles = [
    plt.Line2D([0], [0], marker='o', color=group_1_color,
               label=f"{group_1_name}", markersize=8, linestyle='None'),
    plt.Line2D([0], [0], marker='o', color=group_2_color,
               label=f"{group_2_name}", markersize=8, linestyle='None')
]
...
legend = plt.legend(handles=handles, loc="upper right", frameon=False,
                    prop=legend_font, handletextpad=0.5, borderpad=0.8,
                    labelspacing=0.5, fontsize=12)

# ✅ Save the Plot
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "Fig3H_intensity_median_histogram.svg"),
            format="svg", dpi=300)
plt.savefig(os.path.join(plot_dir, "Fig3H_intensity_median_histogram.png"),
            format="png", dpi=300)
plt.show()
